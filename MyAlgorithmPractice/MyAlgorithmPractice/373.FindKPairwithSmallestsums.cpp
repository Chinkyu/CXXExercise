...
public:
	vector<vector<int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k)
	{
		...
		while (minHeap.size() && k--)
		{
			...
			v.push_back(vector<int> {minHeap.top().first, minHeap.top().second});
			minHeap.pop();
		}
		return v;
	}
...
public:
	vector<vector<int>> kSmallestPairs(vector<int>& nums1, vector<int>& nums2, int k) {
		...
		for (int i = 0; i < k && i < nums1.size(); ++i) {
			int j = 0;
			for (j = 0; j < k && j < nums2.size(); ++j) {
				if (nums1[i] + nums2[j] > max && low_cnt < k) {
					max = nums1[i] + nums2[j];
					low_cnt++;
				}
				else if (nums1[i] + nums2[j] < max && low_cnt < k) {
					low_cnt++;
				}
				//else if (low_cnt > k) {
				//	break;
				//}
				if (low_cnt >= k && nums2[j] > max - nums1[i]) break;

				t.push_back(vector<int>{nums1[i], nums2[j], nums1[i] + nums2[j]});
				//vector<int> vt = { nums1[i], nums2[j], nums1[i] + nums2[j] };
				//t.push_back(vt);
			}
			//if (first_max == INT_MAX) first_max = nums1[i] + nums2[j - 1];
		}
		...
		for (int i = 0; i < k && i < t.size(); ++i) {
			ans.push_back(vector<int> {t[i][0], t[i][1]});
			//vector<int> vt = { t[i][0], t[i][1] };
			//ans.push_back(vt);
		}
		return ans;
	}
