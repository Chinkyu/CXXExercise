...
/*
  vector<string> getNeighbors(const string &word) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      for (int j = 0; j < 26; ++j) {
        tmpWord[i] = char('a' + j);
        if (wordSet.find(tmpWord) != wordSet.end() && tmpWord != word) {
          neighbors.push_back(tmpWord);
        }
      }
    }
    return neighbors;
...
public:
	bool isOneCharDiff(string a, string b) {
		...
		for (int i = 0; i < len; ++i) {
			if(a[i] != b[i]) misMatchCnt++;
		}

		return (misMatchCnt == 1) ? true : false;
	}

	int ladderLength_v01(string beginWord, string endWord, vector<string>& wordList) {
		...
		for (int i = 0; i < len; ++i) {
			...
			if (wordList[i] == endWord) isEndWordBeExist = true;
		}

		if (false == isEndWordBeExist) return 0;
		...
		for (int i = 0; i < len; ++i) {
			...
			if (isOneCharDiff(wordList[i], endWord)) {
				m[wordList[i]].push_back(endWord);
			}
			v[wordList[i]] = false;
		}

		// do bread first search to find shortest path
		queue<string> pq, qq;
		...
		do {
			...
			while (!pq.empty()) {
				...
				for (auto it : m[s]) {
					...
					if (v[it] == true) continue; // already visited pass
					qq.push(it);
				}
			}
		...
		return 0;  // If we got here there is no answer... 
	}

	vector<string> getNeighbors(const string &word, const unordered_map<string, bool> &wordSet) {
		vector<string> neighbors;
		int n = word.length();
		string tmpWord;
		for (int i = 0; i < n; ++i) {
	...
	int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
		...
		// Build graph
		m[beginWord] = getNeighbors(beginWord, v);

		for (int i = 0; i < len; ++i) {
			m[wordList[i]] = getNeighbors(wordList[i], v);
		}
...
int main() {
	...
	//cout << sol.ladderLength("a", "z", wordList);
