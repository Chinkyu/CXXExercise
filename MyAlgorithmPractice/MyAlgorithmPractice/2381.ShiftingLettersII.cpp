...
public:
	...
	// time out
	string _shiftingLetters(string s, vector<vector<int>>& shifts) {
	...
	string shiftingLetters(string s, vector<vector<int>>& shifts) {
		...
		for (int i = 0; i < ans.size(); ++i) {
			if (nm.find(i) != nm.end()) adjust += nm[i];
			if (pm.find(i) != pm.end()) adjust += pm[i];

			adjust %= 26;
			if (ans[i] + adjust < 'a') {
				//ans[i] = 'z' - (adjust - (ans[i] - 'a'));
				ans[i] = 'z' - ('a' - (ans[i] + adjust)) + 1;
			}
			else if (ans[i] + adjust > 'z') {
				ans[i] = 'a' + ((int)(ans[i] + adjust) - 'z') - 1;
			}
			else {
				ans[i] = ans[i] + adjust;
			}
